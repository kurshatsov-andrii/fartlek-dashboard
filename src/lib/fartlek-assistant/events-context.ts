...
export function buildEventsCatalogContext(
  ...
  const lines: string[] = [
    ...
    "Поле «Категорія» узгоджене із фільтром «Події»: marathon = «Біг» (дорожній/асфальтовий забіг узагалі, без обов’язку 42 км), trail = «Трейл», ultra = «Ультра», cycling = «Велоспорт», swimming = «Плавання», triathlon = «Триатлон», duathlon = «Дуатлон», kids = «Дитячий» (лише якщо в дописі немає інших бігових дистанцій поруч із дитячими — напр. «10 км, 5 км, дитячий забіг» це «Біг»), obstacle = «Перешкоди» (OCR: тег #OCR або перешкоди в описі, напр. «Дика гонка»).",
  ];
